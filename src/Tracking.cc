...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    // Run YOLO every YOLO_PERIOD frames to reduce CPU/memory bandwidth contention.
    // YOLO thread is fully async — tracking never blocks waiting for YOLO output.
    constexpr int YOLO_PERIOD = 3;
    if(mFrameNum % YOLO_PERIOD == 1)
        mpYOLO->InsertInput(mImRGB, mImDepth2);

    // Dynamic Tracking — non-blocking: consume any ready YOLO output, never wait
    {
        std::vector<cv::Mat> outputYOLO = mpYOLO->GetOutput();
        if(outputYOLO.size() == 2)
        {
            if(mFrameNum == 1)
            {
